 ...
 */
abstract class KestrelHandler(val memAction: HandlerFunction, val maxOpenTransactions: Int) {
  private val log = Logger.get(getClass.getName)

  val sessionId = MemcachedService.sessionId.incrementAndGet()
  val finished = new AtomicBoolean(false)

  MemcachedService.sessions.incrementAndGet()
  ...
  protected def finish() {
    ...
    if (finished.getAndSet(true) == false) {
      ...
      MemcachedService.sessions.decrementAndGet()
    }
  }
