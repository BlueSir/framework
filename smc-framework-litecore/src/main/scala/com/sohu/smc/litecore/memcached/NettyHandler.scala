...
extends KestrelHandler(memAction, maxOpenTransactions) with ChannelUpstreamHandler {
      ...
      case s: ChannelStateEvent =>
        if ((s.getState() == ChannelState.CONNECTED) && (s.getValue() eq null)) {
          ...
          if (clientTimeout.isDefined) {
            channel.getPipeline.addFirst("idle", new IdleStateHandler(MemcachedService.kestrel.timer, 0, 0, clientTimeout.get.inSeconds.toInt))
          }
