 ...
 */
abstract class NettyHttpHandler[M](
  val channelGroup: ChannelGroup,
  httpController: Controller,
  clientTimeout: Option[Duration])
extends NettyHandler(httpController) with ChannelUpstreamHandler {
  ...
  protected def clientDescription: String = {
    "%s:%d".format(remoteAddress.getAddress, remoteAddress.getPort)
  }
      ...
      case s: ChannelStateEvent =>
        if ((s.getState() == ChannelState.CONNECTED) && (s.getValue() eq null)) {
          ...
          if (clientTimeout.isDefined) {
            channel.getPipeline.addFirst("idle", new IdleStateHandler(WebService.webservice.timer, 0, 0, clientTimeout.get.inSeconds.toInt))
          }
