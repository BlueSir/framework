...
package com.sohu.smc.opentsdb.core;

import com.sohu.smc.opentsdb.uid.NoSuchUniqueId;
import com.sohu.smc.opentsdb.uid.NoSuchUniqueName;
...
final public class TsdbQuery implements Query {
     ...
     * @throws IllegalArgumentException if bad data was retreived from HBase.
     */
    public void compaction(int COMPACTED_TIMESPAN) throws HBaseException {
        final short metric_width = tsdb.metrics.width();
        final TreeMap<byte[], Span> spans =  // The key is a row key from HBase.
                new TreeMap<byte[], Span>(new SpanCmp(metric_width));
        int nrows = 0;
        ...
        try {
            ArrayList<ArrayList<KeyValue>> rows;
            while ((rows = scanner.nextRows().joinUninterruptibly()) != null) {
                final TreeMap<byte[], AtomicLong> compacted = new TreeMap<byte[], AtomicLong>(new ByteCmp(metric_width));
                //final TreeMap<byte[], AtomicLong> compactedTimes = new TreeMap<byte[], AtomicLong>(new ByteCmp(metric_width));
                ...
                for (final ArrayList<KeyValue> row : rows) {
                    ...
                    if (base_time < start_time || base_time > end_time) {
                    ...
                    for (int i = 0; i < nkvs; i++) {
                        ...
                        short qualifier = (short) ((Bytes.getShort(qual) & 0xFFFF) >>> Const.FLAG_BITS);

                        System.out.println(qualifier + "\t" + Bytes.getLong(kv.value()));

                        qualifier = (short) (((qualifier / COMPACTED_TIMESPAN) * COMPACTED_TIMESPAN) << Const.FLAG_BITS | 0x7);

                        byte[] qual_key = Bytes.fromShort(qualifier);
                        AtomicLong aLong = compacted.get(qual_key);
                        if (aLong == null) {
                            aLong = new AtomicLong();
                            compacted.put(qual_key, aLong);
                        }
                        aLong.addAndGet(Bytes.getLong(kv.value()));
                    ...
                    //删除row
                    //tsdb.delete(row.get(0).key());
                    byte[][] qualifier = new byte[compacted.size()][];
                    byte[][] value = new byte[compacted.size()][];
                    Iterator<byte[]> iterator = compacted.keySet().iterator();
                    int index = 0;
                    while (iterator.hasNext()) {
                        qualifier[index] = iterator.next();
                        value[index] = Bytes.fromLong(compacted.get(qualifier[index]).get());
                        index++;
                    }

                    byte[] b_metric = new byte[metric_width];
                    System.arraycopy(row.get(0).key(), 0 , b_metric, 0, metric_width);
                    int metricType = COMPACTED_TIMESPAN / 60;
                    String metric = tsdb.metrics.getName(b_metric) + "." + metricType +"m";
                    System.out.println(metric);
                    byte[] new_b_metric = tsdb.metrics.getOrCreateId(metric);
                    byte[] new_key = row.get(0).key();
                    System.arraycopy(new_b_metric, 0 , new_key, 0, metric_width);

                    tsdb.put(new_key, qualifier, value);
    ...
    public long devideUp(long a, long b) {
     ...
     * Comparator that ignores timestamps in row keys.
     */
    private static final class ByteCmp implements Comparator<byte[]> {

        private final short metric_width;

        public ByteCmp(final short metric_width) {
            this.metric_width = metric_width;
        }
